jQuery(document).ready(function() {
    ...
    function select_slide() {
        jQuery('.select-enable-hide select').change(function() {
			
            var eleme = jQuery(this).val();

            // Slide up (hide) all sibling elements
            jQuery(this).find('option').each(function(index, element) {
                var sibling = "div[id$='" + jQuery(element).val()+"']";
                jQuery(sibling).stop().stop().slideUp('slow');
            });
            // Show the needed element
            jQuery("div[id$='" + eleme+"']").stop().stop().slideDown('slow');
        });
    }

	jQuery(".kalimah-widget-section-start.collapsible h2").click(function () {
		jQuery(this).siblings('.section_start_container').slideDown();
		jQuery(this).siblings('.collapsible-icon').text("-");
		jQuery(this).closest(".kalimah-widget-section-start").siblings().find('.section_start_container').slideUp();
		jQuery(this).closest(".kalimah-widget-section-start").siblings().find('.collapsible-icon').text("+");
	});
	
	jQuery(".kalimah-widget-section-start.collapsible:first h2").trigger("click");
	});